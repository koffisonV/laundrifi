...
export default function WeeklySchedule({ userSlot: initialUserSlot, bookedSlots: initialBookedSlots }: WeeklyScheduleProps) {
  ...
  const [bookedSlots, setBookedSlots] = useState<BookedSlot[]>(initialBookedSlots.map(slot => ({ slot_time: slot, user_id: '' })));
  const [userSlot, setUserSlot] = useState<string | null>(initialUserSlot || null);
  ...
  useEffect(() => {
    const fetchSlots = async () => {
      // Get current user's slot
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) return;

      const { data: userSlotData } = await supabase
        .from('laundry_slots')
        .select('slot_time')
        .eq('user_id', user.id)
        .eq('status', 'active')
      ...
      if (userSlotData) {
        setUserSlot(userSlotData.slot_time);
      }
  ...
  useEffect(() => {
    const fetchTakenSlots = async () => {
      ...
      if (data) {
        setTakenSlots(data.map(booking => booking.slot_time));
      }
    };
  ...
  const isSlotBooked = (day: string, hour: string) => {
    const slot = `${day}-${hour}`;
    return bookedSlots.some(bookedSlot => bookedSlot.slot_time === slot);
  };
      ...
      {userSlot ? (
        <div className="bg-green-100 dark:bg-green-900 p-4 rounded-lg">
          <p className="text-green-800 dark:text-green-200">
            Your booked slot: {new Date(userSlot).toLocaleString()}
          </p>
        </div>
      ) : (
                  ...
                  <button
                    ...
                    className={`flex-1 bg-blue-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-blue-700 
                      ...
                      ${selectedSlot ? (() => {
                        ...
                        return isSlotBooked(day, hour) ? 'bg-gray-200 text-gray-500' : '';
                      })() : ''}
                      ${selectedSlot ? (() => {
                        const [day, hour] = selectedSlot.split('-');
                        return isUserSlot(day, hour) ? 'bg-green-600' : '';
                      })() : ''}`}
                  >
                    Confirm Booking
                  </button>
